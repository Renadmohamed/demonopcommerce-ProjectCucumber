...
public class D04_searchStepDef {
    ...
    public void iEnterSearchProduct( String item) {
        p03HomePage.Search().sendKeys(item);

    }

    @When("user search with using  {string} on search field")
    public void iEnterSearchWithUsingSKU(String SKu) {
        p03HomePage.Search().sendKeys(SKu);

    }

    @And("user click on search button")
    public void iClickOnSearchButton() {
        p03HomePage.Search_button().click();
